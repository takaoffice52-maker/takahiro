...
LINE_ACCESS_TOKEN = "HJDR3GCHnP5i4Et6S7pIyvUV5u7VNyb3cfL5nndQlUKU+ltgNvnG3xQLtdV7qxglVDib7/ShOKdwRAnU3Rjl976YvNllZ8YbEVYZNQvVA8jWbdwNyM9YB+SJwPUOd8KNS/SNTasJZ2oXloIs2+WlswdB04t89/1O/w1cDnyilFU="
LINE_USER_ID = "U58eb97f473a640fe4c71fe42fef0be1a"

# 絞り込み条件
MIN_AREA_M2 = 50  # 専有面積50㎡以上
            ...
            for item in items:
                try:
                    ...
                    if area is not None and area < MIN_AREA_M2:
                        logger.debug(f"  除外(面積不足): {name} {area_text}")
                        continue
                    ...
                    logger.debug(f"  取得: {name} / {price_text} / {area_text}")
...
def scrape_with_requests(url: str, location: str, debug: bool = False) -> list[dict]:
    ...
    properties = []
    try:
        logger.debug(f"  (requests) アクセス: {url}")
...
MOCK_PROPERTIES = [
    {
        ...
        "area_text": "72.50㎡",
        "location": "那覇市",
        "url": "https://www.e-uchina.net/bukken/mock001",
        "first_seen": datetime.now().isoformat(),
    },
    {
        "id": "mock002",
        ...
        "area_text": "85.00㎡",
        "location": "浦添市",
        "url": "https://www.e-uchina.net/bukken/mock002",
        "first_seen": datetime.now().isoformat(),
    },
    {
        "id": "mock003",
        ...
        "area_text": "65.30㎡",
        "location": "宜野湾市",
        "url": "https://www.e-uchina.net/bukken/mock003",
        "first_seen": datetime.now().isoformat(),
    },
]


def scrape_url(url: str, location: str, debug: bool = False, mock: bool = False) -> list[dict]:
    """URLをスクレイピング (Playwright -> requests -> mock の順で試行)"""
    if mock:
        logger.info(f"  [MOCK] {location}: {len(MOCK_PROPERTIES)}件返却")
        return [p for p in MOCK_PROPERTIES if p["location"] == location]

    # Playwright を試みる
    try:
        import playwright
        results = scrape_with_playwright(url, location, debug)
        return results
    except ImportError:
    ...
    # requests を試みる
    try:
        results = scrape_with_requests(url, location, debug)
        return results
    except Exception as e:
        logger.error(f"  requestsもエラー: {e}")
        return []
...
def format_new(prop: dict) -> str:
    return (
        f"🏠 新着物件\n"
        f"物件名: {prop['name']}\n"
        f"価格: {prop['price_text']}\n"
        f"面積: {prop['area_text']}\n"
        f"場所: {prop['location']}\n"
        f"👉 {prop['url']}"
    )
...
def format_price_change(prop: dict, old_price_text: str) -> str:
    return (
        f"💰 価格変更\n"
        f"物件名: {prop['name']}\n"
        f"価格: {old_price_text} → {prop['price_text']}\n"
        f"面積: {prop['area_text']}\n"
        f"場所: {prop['location']}\n"
        f"👉 {prop['url']}"
    )
...
def format_delisted(prop: dict) -> str:
    return (
        f"❌ 掲載終了/成約済み\n"
        f"物件名: {prop['name']}\n"
        f"価格: {prop['price_text']}\n"
...
# --- メイン処理 ---
def main():
    parser = argparse.ArgumentParser(description="e-uchina.net 物件監視スクリプト")
    parser.add_argument("--debug", action="store_true", help="デバッグモード(詳細ログ)")
    parser.add_argument("--mock", action="store_true", help="モックデータで動作確認")
    parser.add_argument("--no-notify", action="store_true", help="LINE通知を送らない")
    ...
    # ネットワーク確認
    use_mock = args.mock
    if not use_mock and not args.debug:
        # 通常モードではネットワーク確認不要
        pass
    elif args.debug and not use_mock:
        # --debug だけの場合、ネットワーク不可ならmockを自動適用
        try:
            import requests
            requests.get("https://www.e-uchina.net", timeout=5)
        except Exception:
            logger.warning("ネットワーク到達不可。--mock モードで動作します。")
            use_mock = True
    ...
    for location, url in URLS:
        ...
        props = scrape_url(url, location, debug=args.debug, mock=use_mock)
    ...
    for pid, prop in all_current.items():
        if pid not in old_data:
            logger.info(f"[新着] {prop['name']} / {prop['price_text']} / {prop['location']}")
            notifications.append(("new", prop, None))
        else:
            old = old_data[pid]
            if prop["price"] != old.get("price") and prop["price"] is not None:
                logger.info(
                    f"[価格変更] {prop['name']}: {old.get('price_text','?')} → {prop['price_text']}"
                )
    ...
    for pid, old_prop in old_data.items():
        if pid not in all_current and not old_prop.get("delisted"):
            logger.info(f"[掲載終了] {old_prop['name']} / {old_prop.get('location','?')}")
    ...
    if notifications and not args.no_notify:
        for ntype, prop, extra in notifications:
            ...
            logger.info(f"LINE通知送信: {ntype}")
            if args.debug or use_mock:
                logger.debug(f"  メッセージ:\n{msg}")
            else:
                send_line(msg)
